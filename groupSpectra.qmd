...
```{r}
tData <- read.csv('formattedData/spectraMrWhiff.csv', fill=TRUE)
...
tData <- downsampleData(tData, 
                        fraction=downsample_factor, 
                        variables=vars) %>% compressData(k=compress_factor)
```

### OPTIONAL - Exclude rows

You may want to exclude some rows from your data based on certain conditions in your metadata - you can do that in the code block below by specifying the metadata variable to filter by as well as the condition to filter by.

```{r}
filter_variable <- list('Amb_Temp_C') #🟢
filter_type <- list('range') #🟢
filter_specification <- list(c(10,35)) #🟢

tData <- filterData(tData,
...
xScale <- NULL #🟢
xLims <- NULL #🟢
XYlabs <- c('Temperature', 'Resistance') #🟢
...
landmark <- TRUE   #🟢 toggle this to TRUE to use the landmark method
nLandmarks <- 150   #🟢 only used when landmark = TRUE
landmarkSeed <- 1   #🟢 

distResult <- getDistMat(tData, fixed = fixed, filters = filters, 
                         iters = iterations, landmark = landmark, 
                         nLandmarks = nLandmarks, seed = landmarkSeed)

if (distResult$type == "full") {
  mds_result <- cmdscale(distResult$DM, k = 2)
  mds_coords <- as.data.frame(mds_result)
  colnames(mds_coords) <- c("MDS1", "MDS2")
} else {
  cmd <- cmdscale(distResult$Dl, k = 2, eig = TRUE)
  L <- cmd$points          # nl x 2 landmark coordinates
  eigvals <- cmd$eig[1:2]  # top 2 eigenvalues

  Dl2 <- distResult$Dl^2
  mean_d2 <- rowMeans(Dl2)               # nl-length vector

  Lplus <- sweep(t(L), 1, eigvals, "/")  # 2 x nl (Gower pseudo-inverse)

  Dall2 <- distResult$Dall^2                       # n x nl
  centred <- sweep(Dall2, 2, mean_d2, "-")         # n x nl
  coords <- -0.5 * (centred %*% t(Lplus))          # n x 2

  mds_coords <- as.data.frame(coords)
  colnames(mds_coords) <- c("MDS1", "MDS2")
}
